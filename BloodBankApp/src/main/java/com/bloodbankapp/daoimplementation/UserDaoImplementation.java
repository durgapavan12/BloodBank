...
public class UserDaoImplementation implements UserDao {
	...
	public Response registration(Registration registration) throws BloodBankException {
		...
		if((registration.getName()!=null)&&(registration.getPassword()!=null)&&(registration.getBloodGroup()!=null)&&
				(registration.getName()!="")&&(registration.getBloodGroup()!="")&&(registration.getPassword()!="")
				&&(registration.getAge()!=0)&&(registration.getPhNo()!=0)&&(registration.getGender()!=null)&&(registration.getGender()!=""))
				{
		...
		public Login loginCheck(Login login) throws BloodBankException {
			Login user=null;
			 ...
			 if((login.getPhNo()!=0)&&(login.getPassword()!=null)&&(login.getPassword()!="")) {
				 try {
						 user = new Jongo(DB_Constants.getMongodbDatabase

			()).getCollection(DB_Constants.getRegistrationCol())
								.findOne("{phNo:#}", login.getPhNo()).as

			(Login.class);
					
						if (user != null) {
							String generatedlogedPasswordHash = BCrypt.hashpw

			(login.getPassword(), BCrypt.gensalt(12));
							boolean b = BCrypt.checkpw(user.getPassword(), 

			generatedlogedPasswordHash);
							if (b) {
						...
						throw new BloodBankException("Exception occured while login",e);
					}
				 
			 }else
			 {
				 user.setStatusCode(ResponseConstants.Error_code);
					user.setStatusMessage("fields empty"); 
			 }
			 return user;
	...
	public Response changePassword(String newPassword, String oldPassword, long phNo) throws BloodBankException {
		...
		Response response = new Response();
		if((newPassword!=null)&&(newPassword!="")&&(oldPassword!=null)&&(oldPassword!="")&&(phNo!=0)){
		try {
		Registration dbprofile = new Jongo(DB_Constants.getMongodbDatabase())
